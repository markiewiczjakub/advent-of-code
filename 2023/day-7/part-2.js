...
function valuesNoJoker(cardsCount) {
	return Object.keys(cardsCount)
		.filter((card) => card !== 'J')
		.map((card) => cardsCount[card]);
}

function isFive(cardsCount) {
	if (cardsCount['J'] === 5) return true;

	return Object.values(cardsCount).some(
		(value) => value === 5 - (cardsCount['J'] ?? 0)
	);
}

function isFour(cardsCount) {
	return valuesNoJoker(cardsCount).some(
		(value) => value === 4 - (cardsCount['J'] ?? 0)
	);
}
...
function isFull(cardsCount) {
	if (cardsCount['J'] === 1) {
		return (
			Object.values(cardsCount).filter((value) => value === 2).length ===
			2
		);
	}
...
function isTwo(cardsCount) {
	if (cardsCount['J'] === 1) {
		return (
			Object.values(cardsCount).some((value) => value === 2) &&
			valuesNoJoker(cardsCount).some((value) => value === 1)
		);
	}
